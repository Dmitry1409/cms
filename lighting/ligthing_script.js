window.addEventListener('DOMContentLoaded',()=>{
	...
	async function basketAction(){
		...
		let res = await fetch(`addProdPersAcc.php?objProduct=${JSON.stringify(obj)}`)
		if(res.ok){
	...
	async function getAllProduct(){
		let res = await fetch("getToledoProducts.php?comand=getAllProd")
		if(res.ok){
	...
	async function getToledoData(){
		...
		query = `getToledoProducts.php?category=${JSON.stringify(cat)}`
