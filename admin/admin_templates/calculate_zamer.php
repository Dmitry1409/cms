<?php
	...
	$all_mat = [
			...
			"комплект крючков для Lumfer"=>0,
			"гардина ПК-5 белый"=>0,
			"гарпун"=>0,
			"гарпун двойной"=>0,
			"торцевая заглюшка Lumfer UK белый"=>0,
			"торцевая заглюшка Lumfer UK черный"=>0,
			"маскировочная вставка в Lumfer белый"=>0,
			"маскировочная вставка в Lumfer черный"=>0,
			"блок питания 50W"=>0,
			"блок питания 100W"=>0,
			"блок питания 150W"=>0,
			"профиль line30"=>0,
				...
				case "гардина накладная":
					foreach ($value as $vrag) {
						...
						if(!$vrag->{'гардина клиента'}){
							if(array_key_exists("ПВХ гардина 2-х рядная"." ".$vrag->{'м.п'}." м.", $all_mat)){
								$all_mat["ПВХ гардина 2-х рядная"." ".$vrag->{'м.п'}." м."] += 1;
							}else{
								$all_mat["ПВХ гардина 2-х рядная"." ".$vrag->{'м.п'}." м."] = 1;
							}
				...
				case "гардина скрытая":
					foreach ($value as $vhid) {
						...
						if($vhid->{"тип"} == "ПК-5"){
							...
							$all_mat['комплект крючков для Lumfer'] += 1;
							$all_mat['маскировочная вставка в Lumfer '.$vhid->{"цвет"}] += ceil($vhid->{"м.п"} *2);
							if($vhid->{"обрыв"} != ""){
