window.addEventListener("DOMContentLoaded", ()=>{
	...
	class DayShow{
		...
		get_select_html(type, status){
			let dict = {
				"замер": ["будет","перенесено","отменено"],
				"монтаж": ["назначен","выполнен","перенесен", "отменен"],
				"вх. звонок": ["было", "обработан"],
				"ис. звонок": ["будет", "перенесен","обработан","не дозвониться"],
				"заказать": ["ожидает отправки", "отправлен", "отменен"],
				"доставка": ["будет","отменено","перенесено", "выполнено"],
				"другое": ["не выполнено", "выполнено"]
			}
			let html = ""
